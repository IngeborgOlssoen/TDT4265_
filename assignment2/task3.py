...
def calculate_accuracy(
    ...
    output_model = model.forward(X)

   # Turn model output into one-hot encoded predictions
    peak_indices = np.argmax(output_model, axis=1)
    one_hot_predictions = np.zeros_like(output_model)

    for index, one_hot_row in zip(peak_indices, one_hot_predictions):
        one_hot_row[index] = 1

    total_samples = X.shape[0]
    correct_count = 0

    # Compare one-hot predictions to actual targets
    for prediction_vector, actual_vector in zip(one_hot_predictions, targets):
        if np.array_equal(prediction_vector, actual_vector): correct_count += 1

    prediction_accuracy = correct_count / total_samples
...
class SoftmaxTrainer(BaseTrainer):
    ...
    def train_step(self, X_batch: np.ndarray, Y_batch: np.ndarray):
        ...
        # Execute the forward pass using the current batch
